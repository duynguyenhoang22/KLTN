# %% [markdown]
# # Setup E — Real + Synthetic Label 1 augmentation (PhoBERT)
#
# **Phương pháp:** Train on Real + Synthetic Label 1, Test on Real  
...
# **Synthetic augmentation:** thêm synthetic Label 1 theo nhiều mức E1/E2/E3/E4  
# **Báo cáo:** mean ± std trên Real Test Set
#
# ---
# **Mục tiêu Setup E:** kiểm tra synthetic Label 1 có giá trị augmentation khi Real Train
# giữ vai trò neo miền thật hay không.
#
# **Biến thể:**
# - **E1:** Real Train + 500 synthetic Label 1
# - **E2:** Real Train + 1000 synthetic Label 1
# - **E3:** Real Train + 2000 synthetic Label 1
# - **E4:** Real Train + toàn bộ synthetic Label 1 sạch sau leakage check
#
# **Chỉ số chính:**
# - Macro-F1
# - F1 Label 1
...
CFG = dict(
    ...
    # Synthetic Label 1 augmentation
    synthetic_sample_seed   = 0,
    variants_to_run         = ["E1_500", "E2_1000", "E3_2000", "E4_all"],
    variant_sizes           = {
        "E1_500": 500,
        "E2_1000": 1000,
        "E3_2000": 2000,
    ...
    output_dir              = "/content/drive/MyDrive/KLTN/setup_e_results",
    work_dir                = "/content/tmp_checkpoints_setup_e",
)
...
#
# Setup E dùng Real Train đầy đủ và chỉ bổ sung synthetic Label 1.
...
print("\nSynthetic origins dùng cho Setup E:")
...
#
# Nếu setup_a_real_test.csv tồn tại, Setup E dùng chính file đó làm Real Test Set đã khóa.
...
if os.path.exists(setup_a_test_path):
    ...
    print("          Setup E sẽ dùng Real Test Set tái tạo từ split_seed=0.")
...
#
# Setup E loại exact duplicate và normalized duplicate trước khi chọn synthetic Label 1.
...
leakage_path = os.path.join(CFG["output_dir"], "setup_e_leakage_report.csv")
...
# %%
# Cell 7 — Tạo train dataframe cho từng biến thể E
#
# Synthetic Label 1 được sample lồng nhau bằng cùng một thứ tự shuffle cố định:
# E1 là prefix của E2, E2 là prefix của E3, và E4 dùng toàn bộ synthetic Label 1 sạch.
...
for variant_name in CFG["variants_to_run"]:
    requested_size = CFG["variant_sizes"][variant_name]
    if requested_size is None:
        n_synthetic = len(synthetic_label1_pool)
    else:
        n_synthetic = int(requested_size)
        assert n_synthetic <= len(synthetic_label1_pool), (
            f"{variant_name} cần {n_synthetic} synthetic Label 1, "
            f"nhưng chỉ có {len(synthetic_label1_pool)} mẫu sạch."
        )

    synthetic_part = synthetic_label1_pool.head(n_synthetic).copy()
    train_frame = (
    ...
    out_path = os.path.join(CFG["output_dir"], f"setup_e_train_{variant_name}.csv")
    ...
    variant_metadata_rows.append({
        ...
        "requested_synthetic_label1": requested_size if requested_size is not None else "all",
        "actual_synthetic_label1": n_synthetic,
    ...
    print(f"  Synthetic Label 1 thêm vào: {n_synthetic}")
    print(f"  Train file: {out_path}")

variant_metadata = pd.DataFrame(variant_metadata_rows)
metadata_path = os.path.join(CFG["output_dir"], "setup_e_variant_metadata.csv")
...
def train_one_seed(variant_name: str, seed: int, train_ds, val_ds, test_ds):
    print(f"\n{'='*80}")
    print(f"  VARIANT {variant_name} | SEED {seed} — Setup E: Real + Synthetic Label 1")
    print(f"{'='*80}")
    ...
    save_prediction_outputs(
        ...
        f"setup_e_{variant_name}_seed_{seed}_real_test",
        setup_name="E_real_plus_synthetic_label1", seed=seed, variant_name=variant_name,
    ...
    clean["setup"]            = "E_real_plus_synthetic_label1"
...
print("\n" + "="*82)
print("  SETUP E — Real + Synthetic Label 1 augmentation (PhoBERT)")
print("  Train: Real Train + Synthetic Label 1 | Validation/Test: Real")
...
per_seed_path = os.path.join(CFG["output_dir"], "setup_e_per_seed_results.csv")
...
for variant_name in CFG["variants_to_run"]:
    ...
    for k in ALL_METRICS:
        ...
        summary_rows.append({
            "setup": "E_real_plus_synthetic_label1",
            "variant": variant_name,
            "metric": k,
            "mean": round(mean, 4),
            "std": round(std, 4),
...
results_path = os.path.join(CFG["output_dir"], "setup_e_results.csv")
...
def plot_aggregated_confusion_matrix(variant_name):
    ...
    fig.suptitle(
        f"Setup E — {variant_name} Real + Synthetic Label 1\n"
    ...
    cm_path = os.path.join(CFG["output_dir"], f"setup_e_confusion_matrix_{variant_name}.png")
